...
@Ignore
class TestRepositoryImpl(val vc: VersionControl) extends TestRepository{
	...
	def clone(repo: TestRepository) {
	  assertTrue(repo.isInstanceOf[TestRepositoryImpl])
	  vc.clone(repo.dir.getCanonicalPath())
	}
